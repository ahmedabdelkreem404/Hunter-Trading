...
export default function HeroVideo({ settings = {}, title = 'Hunter Trading' }) {
  ...
  return (
    <div className="relative h-full min-h-[320px] overflow-hidden rounded-[2rem] border border-white/10 bg-hunter-card shadow-[0_28px_70px_rgba(0,0,0,0.24)] sm:min-h-[440px] lg:min-h-[600px]">
      {videoUrl ? (
        <video
          key={videoKey}
        ...
        <img src={fallbackImage} alt={title} className="absolute inset-0 h-full w-full object-cover" loading="eager" />
      ) : (
        <div className="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_top,_rgba(0,255,136,0.16),_transparent_40%),linear-gradient(135deg,_rgba(0,102,255,0.18),_rgba(18,18,26,1))]">
          <div className="flex h-20 w-20 items-center justify-center rounded-full border border-white/15 bg-white/10 text-hunter-green backdrop-blur">
